...
public class ServiceSEWS001002Test extends ServiceEFWS001Test {

	private static final Logger logger = LoggerFactory.getLogger(ServiceSEWS001002Test.class);
	@Autowired
	...
	public Object  handleRequestTest(RequestComType request, Integer statusCode) throws Exception {
		...
		RouteDefinition routeA = camelContext.getRouteDefinition(SEWSReqCode_001.MAILGUN);

		routeA.adviceWith(camelContext, new AdviceWithRouteBuilder() {
			@Override
			public void configure() throws Exception {
				weaveById(SEWSReqCode_001.MAILGUN + "-CALL").replace().to(mockEndpoint01)
	...
	public void sucessTest01() throws Exception {
		...
		String payload = new String(Files.readAllBytes(Paths.get("./src/test/resources/payload/SEWS-001-002/request001.json")));

		RequestComType request = (RequestComType) this.messageJson.unmarshallerJsonRequest(payload);

		ResponseComType response = (ResponseComType) this.handleRequestTest(request, 200);
